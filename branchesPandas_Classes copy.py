...
class branch:
    ...
    @staticmethod
    def create_branch_from_postgres_to_mongo():
        ...
        try:
            ...
            query = "SELECT * FROM branch_test"
            df_postgres = pd.read_sql(query, engine)
            data_dict = df_postgres.to_dict(orient='records')
        ...
        # Load environment variables from .env file
            load_dotenv(dotenv_path='C:\\Users\\sandh\\OneDrive\\Documents\\Python\\Datamodel\\Basics\\.env')

            uri = os.getenv("db_mongo")
            if not uri:
                raise ValueError("Environment variable 'db_mongo' not set")
...
branch.create_branch_from_postgres_to_mongo()